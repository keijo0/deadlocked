...
use crate::{
    constants::{cs2, elf},
    performance::{MemoryCache, ReadRequest, parallel_reader},
};

/// A file handle used for bulk memory reads via `read_at` (e.g. pattern scanning).
#[derive(Debug)]
...
impl Process {
    pub fn new(pid: i32) -> Self {
        if pid == -1 {
            return Self {
                ...
                min: u64::MAX,
                max: u64::MIN,
                mem_cache: RefCell::new(MemoryCache::new()),
            };
        }
    ...
    /// Evict stale entries from the internal memory cache.  Call once per game
    /// tick so that the next frame starts with only fresh data.
    pub fn evict_cache(&self) {
        self.mem_cache.borrow_mut().evict_expired();
    }

    /// Read `T` from `address`, returning the cached value if a fresh entry
    /// exists, otherwise issuing a new `process_vm_readv` and caching the
    /// result.
    pub fn read_cached<T: Pod + Default>(&self, address: u64) -> T {
        let size = size_of::<T>();
        if let Some(bytes) = self.mem_cache.borrow().get(address) {
            if bytes.len() == size {
                let mut t = T::default();
                bytemuck::bytes_of_mut(&mut t).copy_from_slice(bytes);
                return t;
            }
        }
        let t = self.read::<T>(address);
        self.mem_cache
            .borrow_mut()
            .insert(address, bytemuck::bytes_of(&t).to_vec());
        t
    }

    /// Perform multiple memory reads in parallel using the Rayon thread pool.
    /// Results are returned in the same order as `requests`.
    pub fn read_parallel(&self, requests: &[ReadRequest]) -> Vec<crate::performance::ReadResult> {
        parallel_reader::read_parallel(self.pid, requests)
    }

    /// Write a value of type T to the specified address in the target process.
    pub fn write_memory<T: Pod>(&self, address: u64, value: T) {
        let buffer = bytemuck::bytes_of(&value);

        let local_iov = iovec {
            iov_base: buffer.as_ptr() as *const libc::c_void as *mut libc::c_void,
            iov_len: buffer.len(),
        };
        let remote_iov = iovec {
            iov_base: address as *mut libc::c_void,
        ...
        unsafe {
            nix::libc::process_vm_writev(self.pid, &local_iov, 1, &remote_iov, 1, 0);
        }
    }
