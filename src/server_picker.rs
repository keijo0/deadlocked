...
fn run_iptables(args: &[&str], ip: &str, action: &str) {
    ...
    // Try doas first (OpenBSD / some Linux distros), then sudo, then fall
    // back to direct invocation (works when already root).
    let doas = find_binary(&["/usr/bin/doas", "/usr/local/bin/doas", "doas"]);
    ...
    } else if let Some(ref sudo_path) = sudo {
