...
impl Bvh {
    ...
    /// Count the number of distinct wall/surface clusters a ray passes through between
    /// `start` and `end`. Intersections within 10 world-units of each other along the
    /// ray are merged into a single "wall" cluster.
    pub fn count_wall_intersections(&self, start: Vec3, end: Vec3) -> u32 {
        let dir = end - start;
        ...
        if let Some(root) = self.root {
            self.collect_intersections(root, start, dir_norm, inv_dir, distance, &mut ts);
        }
        ...
        // Cluster intersections that are within 10 world-units of each other into one "wall".
        const CLUSTER_DIST: f32 = 10.0;
        let mut walls: u32 = 1;
        let mut prev = ts[0];
        for &t in &ts[1..] {
            if t - prev > CLUSTER_DIST {
                walls += 1;
            }
            prev = t;
        }
    ...
    fn collect_intersections(
        ...
        match node {
            ...
            BvhNode::Branch { left, right, .. } => {
                self.collect_intersections(*left, origin, direction, inv_dir, max_t, ts);
                self.collect_intersections(*right, origin, direction, inv_dir, max_t, ts);
            }
        }
    }
