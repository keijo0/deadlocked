    ...
    ui.group(|ui| {
        ui.horizontal(|ui| {
            ...
            for option in T::iter() {
                ...
                if ui
                    .add(egui::Button::new(text).selected(selected).frame(true))
                    .clicked()
                    && !selected
                {
                    *value = option;
...
/// Renders a binary enum as two adjacent framed buttons with clear active/inactive distinction.
pub fn toggle_buttons<T: std::fmt::Debug + strum::IntoEnumIterator + PartialEq>(
    ui: &mut Ui,
    label: &str,
    value: &mut T,
) -> bool {
    combo_box(ui, label, value)
}
...
pub fn keybind_list(ui: &mut Ui, id: &str, label: &str, keycodes: &mut Vec<KeyCode>) -> bool {
    let mut changed = false;
    ui.group(|ui| {
        // Label + small add button on the same row
        ui.horizontal(|ui| {
            ui.label(label);
            if keycodes.len() < 5 && ui.small_button("+").clicked() {
