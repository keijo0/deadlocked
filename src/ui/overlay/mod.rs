...
impl App {
    ...
    pub fn overlay(&mut self, ui: &mut Ui) {
        ...
        for player in &data.players {
            ...
            self.draw_backtrack(&painter, player, data);
        }
        ...
        self.draw_fov_arrows(&painter, data);
        self.draw_sniper_crosshair(&painter, data);
        self.draw_penetration_crosshair(&painter, data);
        self.draw_spectator_list(&painter, data);
        self.draw_keybind_list(&painter, data);
        self.draw_media_info(&painter, data);
        self.draw_watermark(&painter, data);

        if self.config.hud.keybind_list && self.config.hud.keybind_trigger_active_indicator && data.triggerbot_active {
            self.text(
                &painter,
                "trigger active",
                pos2(
        ...
        if self.config.hud.keybind_list && self.config.hud.keybind_autowall && data.autowall_active {
            let trigger_offset = if self.config.hud.keybind_trigger_active_indicator && data.triggerbot_active {
            ...
            self.text(
                ...
                pos2(
                    data.window_size.x / 2.0 + 8.0,
                    data.window_size.y / 2.0 + 8.0 + trigger_offset,
                ),
                Align2::LEFT_TOP,
                None,
            );
    ...
    pub(super) fn text_with_font(
        ...
        let color = color.unwrap_or(self.config.hud.text_color);
        if self.config.hud.text_outline {
            for (pos, c) in outline(position, color) {
                painter.text(pos, align, text.as_ref(), font.clone(), c);
            }
        } else {
