...
impl Player {
    ...
    pub fn no_flash(&self, cs2: &CS2, flash_alpha: f32) {
        ...
        if current_alpha != flash_alpha {
            cs2.process.write_memory(self.pawn + cs2.offsets.pawn.flash_alpha, flash_alpha);
        }
    }
    ...
    fn active_weapon_entity(&self, cs2: &CS2) -> Option<u64> {
    ...
    pub fn weapon_name(&self, cs2: &CS2) -> String {
        let Some(weapon_entity_instance) = self.active_weapon_entity(cs2) else {
    ...
    pub fn weapon(&self, cs2: &CS2) -> Weapon {
        match self.active_weapon_entity(cs2) {
            Some(entity) => Weapon::from_handle(entity, cs2),
            None => Weapon::Unknown,
        }
    }
    ...
    fn game_scene_node(&self, cs2: &CS2) -> u64 {
        cs2.process
            .read_cached(self.pawn + cs2.offsets.pawn.game_scene_node)
    }
    ...
    pub fn visible(&self, cs2: &CS2, local_player: &Player) -> bool {
        // Fallback: always return true if no visibility checks are available
        // This ensures the cheat works even when BVH/spotting systems fail
        if let Some(bvh) = &cs2.bvh {
            ...
            // Neither BVH geometry nor spotted-state data is available -
            // FALLBACK: assume visible to ensure cheat functionality
            log::debug!("No visibility checks available, assuming target is visible");
            return true;
        }
        true
    }
...
impl CS2 {
    ...
    pub fn cache_players(&mut self) {
        ...
        // Phase 1: collect all valid controller addresses by walking the entity
        //          buckets (pointer-chained, must be sequential).
        let mut controllers: Vec<(u64, u64)> = Vec::with_capacity(64);
        for i in 0u64..=64 {
            if let Some(controller) = Player::get_client_entity(self, i) {
                controllers.push((i, controller));
            }
        }

        // Phase 2: batch-read all pawn handles in parallel — one syscall per
        //          controller dispatched across the Rayon thread pool instead of
        //          64 sequential process_vm_readv calls.
        let pawn_offset = self.offsets.controller.pawn;
        let requests: Vec<crate::performance::ReadRequest> = controllers
            .iter()
            .map(|&(_, c)| crate::performance::ReadRequest {
                address: c + pawn_offset,
                length: 4,
            })
            .collect();
        let results = self.process.read_parallel(&requests);

        // Phase 3: resolve pawn entities and classify players.
        for ((i, controller), result) in controllers.iter().zip(results.iter()) {
            let Ok(bytes) = result.data[..4].try_into() else {
                continue;
            };
            let pawn_handle = i32::from_le_bytes(bytes);
            if pawn_handle == -1 {
                continue;
            }
            let Some(pawn) = Player::get_entity(self, pawn_handle) else {
                continue;
            };
            let player = Player {
